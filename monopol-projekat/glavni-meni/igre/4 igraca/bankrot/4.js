...
async function pokreniAukciju(poljaZaProdaju) {
  ...
  async function procesuirajAukciju() {
    ...
    // Dodaj odmah kupovinu i prekini aukciju za to polje
    sledeciPonudjac.novac -= ponudaBroj;
    sledeciPonudjac.posedi.push(poljeIndex);
    vlasnici[poljeIndex] = sledeciPonudjac.id;

    showBootstrapAlert(`Igrač ${sledeciPonudjac.id} osvojio je polje ${imenaPolja[poljeIndex]} za ${ponudaBroj}$.`);

    aktivniIgraci.forEach(p => p.odustaoOdAukcije = false);

    // Pređi na sledeće polje ako postoji
    poljaZaProdaju.shift();
    if (poljaZaProdaju.length > 0) {
        pokreniAukciju(poljaZaProdaju);
    } else {
    ...
    return; // Završi ovu iteraciju aukcije
} else {
...
window.addEventListener('load', () => {
  ...
  if (testMode) {
    figurice[0].novac = -10;
    figurice[0].posedi = [1, 3];
    vlasnici[1] = 1;
    vlasnici[3] = 1;
    azurirajPrikaz();

    figurice[1].pozicija = 5;
    figurice[1].novac = 1000;
...
if (testMode) {
  // SCENARIO: Igrač 1 bankrotira i njegova polja idu na aukciju
  figurice[0].novac = -10; // Igrač 1 bankrotira
  figurice[0].posedi = [1, 3]; // Ima Beograd i Novi Sad
  vlasnici[1] = 1;
  vlasnici[3] = 1;
  azurirajPrikaz();
  ...
  // Pokreni sledećeg igrača (poziva bankrot aukciju automatski)
  setTimeout(() => {
    sledeciIgrac();
  }, 500); // mali delay da se DOM učita
}